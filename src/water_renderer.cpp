...
void water_renderer::draw_quad(const texture* tex, const rectangle& src, const rectangle& dest, float world_x, float world_y, float square_size) {
  ...
  water_vertex vertices[] = {
      {dest.x + dest.w, dest.y, uv_right, uv_top, world_x + square_size, world_y},
      {dest.x + dest.w, dest.y + dest.h, uv_right, uv_bottom, world_x + square_size, world_y + square_size},
      {dest.x,  dest.y, uv_left, uv_top, world_x, world_y},
      {dest.x + dest.w, dest.y + dest.h, uv_right, uv_bottom, world_x + square_size, world_y + square_size},
      {dest.x, dest.y + dest.h, uv_left, uv_bottom, world_x, world_y + square_size},
      {dest.x, dest.y, uv_left, uv_top, world_x, world_y}
  };

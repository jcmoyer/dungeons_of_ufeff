...
enum move_state {
    IDLE, MOVE_LEFT, MOVE_RIGHT, MOVE_UP, MOVE_DOWN
};
...
struct world {
    ...
    std::size_t spawn_player(uint32_t tile_x, uint32_t tile_y) {
        ...
        e.world_x = tile_to_world(tile_x);
        e.world_y = tile_to_world(tile_y);
        e.type = npc;
        e.set_sprite_id(0);
...
#define ASSIGN_PROP(n) if (name == ""#n) { w.n = val; }
...
inline world load_world(std::string_view filename) {
    ...
    for (tile& x : t.base) --x.id;
    for (tile& x : t.detail) --x.id;
    for (tile& x : t.fringe) --x.id;

...
st_play::st_play(game* g, shared_state* s) : owner{ g }, state { s } {
    ...
    if (ev.type == SDL_KEYDOWN && ev.key.keysym.sym == SDLK_x) {
        if (sub == none) {
            ...
            if (e->is_interactive() && e->active) {
                ...
                script(st_interact_context(this, e));
            }
        } else if (sub == message) {
            sub = none;
        }
...
void st_play::update() {
    ...
    if (sub == map_intro_title) {
        ...
        if (me_timer.expired(state->frame_counter)) {
            ...
            current_music->volume = 1.0f - me_timer.progress(state->frame_counter);
        }
        ...
        if (b_fade_timer.expired(state->frame_counter)) {
            ...
            current_music->volume = 1.0f - b_fade_timer.progress(state->frame_counter);
        }
        ...
        if (b_fade_timer.expired(state->frame_counter)) {
            ...
            current_music->volume = b_fade_timer.progress(state->frame_counter);
        }
        ...
        for (uint32_t x = 0; x < wor.map.width; ++x) {
            ...
            if (t.is_water()) {
                if (wor.map.valid(x - 1, y) && !wor.map.at(x - 1, y).is_water() && random::chance(FOAM_EMIT_RATE)) {
                    foam_em->emit({ x * 16, y * 16 + random::rand_int(0, 16) }, rand_vec2(-0.8, 0, -0.5, 0));
                }
                if (wor.map.valid(x + 1, y) && !wor.map.at(x + 1, y).is_water() && random::chance(FOAM_EMIT_RATE)) {
                    foam_em->emit({ 16 + x * 16, y * 16 + random::rand_int(0, 16) }, rand_vec2(0, 0.8, -0.5, 0));
                }
                if (wor.map.valid(x, y - 1) && !wor.map.at(x, y - 1).is_water() && random::chance(FOAM_EMIT_RATE)) {
                    foam_em->emit({ x * 16 + random::rand_int(0, 16), y * 16 }, rand_vec2(-0.2, 0.2, -0.8, 0));
                }
                if (wor.map.valid(x, y - 1) && wor.map.at(x, y - 1).is_waterfall() && random::chance(3 * FOAM_EMIT_RATE)) {
                    foam_em->emit({ x * 16 + random::rand_int(0, 16), y * 16 }, rand_vec2(-0.2, 0.2, -0.8, 0), 6.f);
                }
                if (wor.map.valid(x, y + 1) && !wor.map.at(x, y + 1).is_water() && random::chance(FOAM_EMIT_RATE)) {
                    foam_em->emit({ x * 16 + random::rand_int(0, 16), 16 + y * 16 }, rand_vec2(-0.2, 0.2, 0, 0.8));
                }
            }
        }
    }
...
void st_play::render(double a) {
    ...
    camera lerp_cam;
    lerp_cam.set_x(lerp<double>(prev_cam.get_view().x, cam.get_view().x, a));
    lerp_cam.set_y(lerp<double>(prev_cam.get_view().y, cam.get_view().y, a));
    ...
    for (int y = min_tile_y; y < max_tile_y; ++y) {
        for (int x = min_tile_x; x < max_tile_x; ++x) {
            ...
            uint32_t tid = t.id;
            if (tid == -1) continue;
            uint32_t ysrc = tid / 32;
            uint32_t xsrc = tid % 32;
    ...
    for (entity& e : wor.ents) {
        if (e.type == et_light) {
            ...
            float d_flicker = e.light_state.light_flicker_radius * cos(e.world_x + e.world_y + state->frame_counter);
            light_cmds.push_back({ e.world_x - lerp_cam.left(), e.world_y - lerp_cam.top(), d_flicker + e.light_state.light_radius });
        }
        ...
        uint32_t dx = lerp<double>(
            e.prev_world_x - lerp_cam.left(),
            e.world_x - lerp_cam.left(),
            a
        );
        uint32_t dy = lerp<double>(
            e.prev_world_y - lerp_cam.top(),
            e.world_y - lerp_cam.top(),
            a
        );
    ...
    if (wor.dark) {
        ...
        for (const auto& cmd : light_cmds) {
            ...
            dest.inflate(cmd.radius * 16, cmd.radius * 16);
...
void st_play::process_scheduled_scripts() {
    ...
    for (size_t i = scheduled_scripts.size() - 1; i != -1; --i) {
        if (scheduled_scripts[i].t.expired(state->frame_counter)) {
            ...
            script(st_interact_context(this, nullptr));
...
void st_play::render_battle_fade(double a) {
    ...
    const double out = clamp(b_fade_timer.progress(state->frame_counter), 0.0, 1.0);
    const double in = 1.0 - out;

    state->quad_render->begin();
    state->quad_render->draw_quad({ 0, 0, INTERNAL_WIDTH, INTERNAL_HEIGHT }, 0, 0, 0, sub == battle_fadeout ? out : in);
...
void st_play::render_map_fadeout() {
    ...
    const double c = clamp(me_timer.progress(state->frame_counter), 0.0, 1.0);
...
void st_play::render_map_intro() {
    ...
    const double c = clamp(mi_timer.progress(state->frame_counter), 0.0, 1.0);
    const double x = 1.0 - c;
...
void st_play::render_mlp_fade() {
    ...
    const double out = clamp(mlp_fade_timer.progress(state->frame_counter), 0.0, 1.0);
    const double in = 1.0 - out;

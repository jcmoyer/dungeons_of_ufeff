...
public:
    frag_shader(std::string_view src) : shader_base(GL_FRAGMENT_SHADER, src) {}
};

class vert_shader : public shader_base {
public:
    vert_shader(std::string_view src) : shader_base(GL_VERTEX_SHADER, src) {}
};
...
public:
    ...
    // moveable
    shader_program(shader_program&& x) : handle(x.handle) {
        x.handle = 0;
    }
